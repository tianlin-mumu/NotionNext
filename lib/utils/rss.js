...
import { getEssayContent } from '@/lib/site-pages/essays'
...
const createFeedContent = async post => {
  if (post?.source === 'local-essay') {
    const essaySlug = post.slug?.replace(/^essay\//, '')
    const essay = essaySlug ? getEssayContent(essaySlug) : null
    return essay?.html || post.summary
  }
