...
import {
  applySiteIdentity,
  createCategoryOptionsFromPages,
  createTagOptionsFromPages,
  mergeOwnedContent
} from '../site-pages/virtual-posts'
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  // 站点基础信息
  const siteInfo = applySiteIdentity(
    getSiteInfo({ collection, block, NOTION_CONFIG })
  )

  const curatedPages = mergeOwnedContent(collectionData, siteInfo)

  // 文章计数
  let postCount = 0

  // 查找所有的Post和Page
  const allPages = curatedPages.filter(post => {
  ...
  const categorySchemaOptions = createCategoryOptionsFromPages(allPages)
  // 所有分类
  const categoryOptions = getAllCategories({
    allPages,
    categoryOptions: categorySchemaOptions
  })
  // 所有标签
  const tagSchemaOptions = createTagOptionsFromPages(allPages)
  const tagOptions =
    getAllTags({
      allPages: allPages ?? [],
